...
$(function () {
	...
	$('#modaltopimg').attr('src', $('#imgfoto').attr('src'));
    ...
    var table = $("#ajax-source").dataTable({
		"bProcessing": true,
		"bServerSide": false,
        "sAjaxSource": "usuario/table_result",
        "sServerMethod": "POST",
		"oLanguage": {
		...
		"aoColumnDefs": [
            {
                "mRender": function (data, type, row) {
                    return "<div class='btn-group' style='width:80px !important'><button type='button' class='btn btn-sm btn-default'>Ação</button><button type='button' class='btn btn-sm btn-default dropdown-toggle' data-toggle='dropdown'><span class='caret'></span></button><ul class='dropdown-menu dropdown-menu-right'><li><a href='usuario/edit/" + data + "' id='editar'><i class='icon ico-pencil'></i>Alterar</a></li><li class='divider'></li><li><a href='usuario/del/" + data + "' id='del' class='text-danger'><i class='icon ico-remove3'></i>Excluir</a></li></ul></div>";				
                },
                "aTargets": [-1]
            },
			{
                "mRender": function (data, type, row) {
                    return "<img src='" + data + "' class='img-circle' width=36>";
                },
                "aTargets": [0]
			},
			{
			...
			{
				"mRender": function (data, type, row) {
					if (data == 1) return "<button type='button' class='btn btn-sm btn-default' id='btnpendente'>Admin</button>";
					if (data == 2) return "<button type='button' class='btn btn-sm btn-default'>Pastor</button>";
					if (data == 3) return "<button type='button' class='btn btn-sm btn-default'>Lider GC</button>";
					if (data == 4) return "<button type='button' class='btn btn-sm btn-default'>Coordenador Consolidação</button>";					
					if (data == 5) return "<button type='button' class='btn btn-sm btn-default'>Consolidador</button>";
					if (data == 9) return "<button type='button' class='btn btn-sm btn-default'>Visitante</button>";															
                },
                "aTargets": [4]					
			}
	...
	$('#removerfoto').click(function() {
		$('#foto').val('');
		$('#imgfoto').attr('src', ROOTPATH + 'content/themes/adminre/image/avatar/avatar.png');
	});
	...
	$('#btnSalvar').click(function() {
		...
		$('#foto').val($('#dst').val());
		$('#imgfoto').attr('src', ROOTPATH + 'content/themes/adminre/image/loading/spinner1.gif');
		
		
		setTimeout(function() {
			
			$('#imgfoto').attr('src', ROOTPATH + $('#dst').val());
		}, 5000);
...
function updateCoords(c)
  ...
  function checkCoords()
  {
	...
	function ajaxPhotoUpload()
	{
		...
		(
			{
				url: ROOTPATH + 'usuario/upload_file',
				secureuri:false,
