...
$(function () {
    ...
    var table = $("#ajax-source").dataTable({
		"bProcessing": true,
		"bServerSide": false,
        "sAjaxSource": "tipoinformacao/table_result",
        "sServerMethod": "POST",
		"oLanguage": {
		...
		"aoColumnDefs": [
            {
                "mRender": function (data, type, row) {
                    return "<div class='btn-group' style='width:80px !important'><button type='button' class='btn btn-sm btn-default'>Ação</button><button type='button' class='btn btn-sm btn-default dropdown-toggle' data-toggle='dropdown'><span class='caret'></span></button><ul class='dropdown-menu dropdown-menu-right'><li><a href='tipoinformacao/edit/" + data + "' id='editar'><i class='icon ico-pencil'></i>Alterar</a></li><li class='divider'></li><li><a href='tipoinformacao/del/" + data + "' id='del' class='text-danger'><i class='icon ico-remove3'></i>Excluir</a></li></ul></div>";				
                },
                "aTargets": [-1]
            }
	...
	$('#btn_new').on('click', function() {
		location.href='tipoinformacao/create'
	});
