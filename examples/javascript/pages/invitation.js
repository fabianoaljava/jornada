...
$(function () {
    ...
    var table = $("#ajax-source").dataTable({
		"bProcessing": true,
		"bServerSide": true,
        "sAjaxSource": "invitation/table_result",
        "sServerMethod": "POST",
		"oLanguage": {
		...
		"aoColumnDefs": [
            {
                "mRender": function (data, type, row) {
                    return "<div class='btn-group' style='width:80px !important'><button type='button' class='btn btn-sm btn-default'>Ação</button><button type='button' class='btn btn-sm btn-default dropdown-toggle' data-toggle='dropdown'><span class='caret'></span></button><ul class='dropdown-menu dropdown-menu-right'><li><a href='invitation/edit/" + data + "' id='editar'><i class='icon ico-pencil'></i>Alterar</a></li><li class='divider'></li><li><a href='invitation/confirm/" + data + "' id='editar'><i class='icon ico-check'></i>Confirmar</a></li><li class='divider'></li><li><a href='invitation/del/" + data + "' id='del' class='text-danger'><i class='icon ico-remove3'></i>Excluir</a></li></ul></div>";				
                },
                "aTargets": [-1]					
            }, 
			{
			...
			{
				"mRender": function (data, type, row) {
					if (data == 0) return "<button type='button' class='btn btn-sm btn-default bgcolor-pendente'>PENDENTE</button>";
					if (data == 1) return "<button type='button' class='btn btn-sm btn-default bgcolor-confirmado'>CONFIRMADO</button>";
					if (data == 2) return "<button type='button' class='btn btn-sm btn-default bgcolor-excluido'>EXCLUÍDO</button>";										
                },
                "aTargets": [5]					
			}
	...
	$('#btn_new').on('click', function() {
		location.href='invitation/create'
	});
...
function updateCoords(c)
  ...
  function checkCoords()
  {
	...
	function ajaxPhotoUpload()
	{
		...
		(
			{
				url: ROOTPATH + 'painel/invitation/upload_file',
				secureuri:false,
